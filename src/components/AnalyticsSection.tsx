...
const AnalyticsSection = () => {
  ...
  const summaries: LeaderPerformanceSummary[] = Array.from(leaderMap.entries()).map(([name, data]) => {
    ...
    return {
      name,
      total_pairings: data.scores.length,
  ...
  if (loading) return <p className="text-sm text-muted-foreground text-center py-8">Loading analytics...</p>;
                ...
                <div key={i} className="rounded-lg border border-border p-3 space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Badge variant="outline" className="text-xs">Avg: {s.avg_compatibility}%</Badge>
                      <Badge variant="outline" className="text-xs">Success: {s.success_rate}%</Badge>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="space-y-3">
              {conflictLeaders.map((s, i) => (
                <div key={i} className="rounded-lg border border-border p-3 space-y-1">
                  <div className="flex items-center justify-between">
                    <span className="font-medium text-sm text-foreground">{s.name}</span>
                    <Badge className="bg-amber-500/10 text-amber-600 text-xs" variant="outline">
      ...
      <Card className="border-border shadow-sm">
        ...
        <CardContent className="space-y-3">
          {summaries.filter((s) => s.improvement_suggestions.length > 0).length === 0 ? (
            ...
            summaries
              .filter((s) => s.improvement_suggestions.length > 0)
              .map((s, i) => (
